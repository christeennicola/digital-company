    ...
    <div class="container-fluid my-5 px-4">
        <div class="row justify-content-center">
            <div class="col-12 col-lg-8 col-xl-7">
                ...
                <!-- لوحة نموذج تحديث الخدمة (Update Card) -->
                <div class="card border-0 shadow-sm" style="border-radius: 8px; overflow: hidden;">
                    ...
                    <div class="card-header text-white p-4 text-center"
                        ...
                        <h4 class="mb-1 fw-bold">Update Service</h4>
                        <p class="mb-0 opacity-75 small">Modify the details below to update your service parameters</p>
                    </div>

                    <!-- جسم النموذج المنظم -->
                    <div class="card-body p-4 p-md-5 bg-white">
                        <!-- في لارافل تأكد من إضافة   داخل الفورم الفعلي -->
                        <form action="{{ route('service.update', $dataToupdate->id) }}" method="POST"
                            class="needs-validation" novalidate>
                            @csrf
                            @method('PUT')
                            <!-- حقل العنوان (Service Title) -->
                            <div class="mb-4">
                                <label for="updateServiceTitle" class="form-label fw-bold text-secondary small">Service
                                    Title</label>
                                <div class="input-group">
                                    <span class="input-group-text bg-light border-end-0 text-muted"><i
                                            class="bi bi-type"></i></span>
                                    <!-- تظهر هنا القيمة الحالية للخدمة من قاعدة البيانات -->
                                    <input type="text" class="form-control bg-light border-start-0"
                                        id="updateServiceTitle" name="title" value="{{ $dataToupdate->title }}"
                                        placeholder="e.g. Web Development" style="font-size: 0.9rem;" required>
                            ...
                            <!-- حقل الوصف (Service Description) -->
                            <div class="mb-4">
                                <label for="updateServiceDescription"
                                    class="form-label fw-bold text-secondary small">Service Description</label>
                                <div class="input-group">
                                    <span
                                        class="input-group-text bg-light border-end-0 text-muted align-items-start pt-2"><i
                                            class="bi bi-file-earmark-text"></i></span>
                                    <!-- يطبع الوصف الحالي مباشرة داخل وسم التكست أريا -->
                                    <textarea class="form-control bg-light border-start-0" id="updateServiceDescription" name="description" rows="5"
                                        placeholder="Write a detailed description explaining your service features..." style="font-size: 0.9rem;" required>{{ $dataToupdate->description }}</textarea>
